from azure_storage.methods import client_prep, create_blob_service_client, create_container, \
...
def test_batch_file_download_integration(mock_args, variables):
    ...
    delete_output_file(output_file=os.path.join(variables.download_path, 'triple_nested_file.txt'))
    delete_output_file(output_file=os.path.join(variables.download_path, 'file_2.txt'))
...
def test_batch_folder_download_integration(mock_args, variables):
    ...
    shutil.rmtree(os.path.join(variables.download_path, 'move_folder'))
    shutil.rmtree(os.path.join(variables.download_path, 'nested_1'))
...
def test_batch_container_download_integration(mock_args, variables):
    ...
    shutil.rmtree(os.path.join(variables.download_path, variables.target_container))
