from azure_storage.methods import \
    ...
    create_container, \
    decrypt_credentials
from azure_storage.azure_automate import \
    file_upload, \
    folder_upload, \
    container_sas, \
    file_sas, \
...
def setup():
    class Variables:
        def __init__(self):
            ...
            self.connection_string = decrypt_credentials(account_name=self.account_name)
...
def test_batch_upload_file_integration(mock_args, variables):
    ...
    variables.container_client = create_container_client(
        variables=variables,
...
@patch('argparse.ArgumentParser.parse_args')
def test_batch_file_copy_integration(mock_args, variables):
    variables.copy_client = create_container_client(
        variables=variables,
        container_name='00000000000container'
    )
    mock_args.return_value = argparse.Namespace(
        account_name=variables.account_name,
        verbosity='info',
        batch_file=os.path.join(variables.batch_path, 'copy_file.tsv')
    )
    arguments = cli()
    file_copy(args=arguments)
    blobs = variables.copy_client.list_blobs()
    assert 'move_files/folder_test_1_copy.txt' in [blob.name for blob in blobs]


@patch('argparse.ArgumentParser.parse_args')
def test_batch_folder_copy_integration(mock_args, variables):
    mock_args.return_value = argparse.Namespace(
        account_name=variables.account_name,
        verbosity='info',
        batch_file=os.path.join(variables.batch_path, 'copy_folder.tsv')
    )
    arguments = cli()
    folder_copy(args=arguments)
    blobs = variables.copy_client.list_blobs()
    assert 'move_folder/file_1.txt' in [blob.name for blob in blobs]


@patch('argparse.ArgumentParser.parse_args')
def test_batch_container_copy_integration(mock_args, variables):
    mock_args.return_value = argparse.Namespace(
        account_name=variables.account_name,
        verbosity='info',
        batch_file=os.path.join(variables.batch_path, 'copy_container.tsv')
    )
    arguments = cli()
    container_copy(args=arguments)
    variables.copy_container_client = create_container_client(
        variables=variables,
        container_name='00000000000container'
    )
    blobs = variables.copy_container_client.list_blobs()
